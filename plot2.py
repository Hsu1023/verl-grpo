...
with open(path, 'r') as f:
    lines = f.readlines()
for content in ['pos_label_ratio', 'time_per_step', 'probe_acc', 'generate', 'prob', 'update', 'early_stop','ref']:  # 'clip_ratio', 'early_stop', 'advantages'] + datasets:
    matches = {}
    for line in lines:
        # if 'Save' in line:
        #     print(line)
        ...
        if content == 'pos_label_ratio':
            ...
            match = re.search(pattern, line)
        elif content == 'probe_acc':
            pattern = r'step:([-+]?\d*\.\d+|\d+).*?actor/probe_acc:np.float64\(([-+]?\d*\.\d+|\d+)\)'            
            match = re.search(pattern, line)
        elif content == 'generate':
            pattern = r'step:([-+]?\d*\.\d+|\d+).*?timing_s/generate_sequences:([-+]?\d*\.\d+|\d+)'
            match = re.search(pattern, line)
        elif content == 'prob':
            pattern = r'step:([-+]?\d*\.\d+|\d+).*?timing_s/old_log_prob:([-+]?\d*\.\d+|\d+)'
            match = re.search(pattern, line)
        elif content == 'ref':
            pattern = r'step:([-+]?\d*\.\d+|\d+).*?timing_s/ref:([-+]?\d*\.\d+|\d+)'
            match = re.search(pattern, line)
        elif content == 'update':
            pattern = r'step:([-+]?\d*\.\d+|\d+).*?timing_s/update_actor:([-+]?\d*\.\d+|\d+)'
            match = re.search(pattern, line)
    ...
    if content == 'pos_label_ratio':
        ...
        # print(len(x))
        # print('pos', y[250:300].mean())
        y = y[a:b]
        print(y.mean())
        print((y * (1-y)).mean())
    if content == 'early_stop':
        print('early', y[a:b].mean())
    if content == 'time_per_step':
        print('time', y[a:b].mean())
    if content == 'probe_acc':
        print(len(x))
        # y=y[250:300]
        print('acc', y.mean())
        print((y*(1-y)).mean())
    if content == 'prob':
        print('prob', y[a:b].mean())
    if content == 'generate':
        print('generate', y[a:b].mean())
    if content == 'ref':
        print('ref', y[a:b].mean())
    if content == 'update':
        # print([i for i, _ in enumerate(y) if _ >63.04 and _ < 63.06])
        print('update', y[a:b].mean())
