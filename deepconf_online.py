...
def process_trace(choice, trace_id, ground_truth):
    ...
    # parsed_answer = str(parsed_answer).strip() if parsed_answer is not None else None
    parsed_answer = str(extracted_answer).strip() if extracted_answer is not None else None 
...
def main():
    ...
    for i, trace in enumerate(final_traces[:3]):
        print(f"  Trace {i}: {trace['extracted_answer']} (correct: {trace['is_correct']}, conf: {trace['min_conf']:.4f}, tokens: {trace['token_count']}, text: {trace['text']})")
        print(f" Stop reason: {trace['stop_reason']}, Finish reason: {trace['finish_reason']}")
    ...
    print(f"Avg tokens per trace: {overall_stats['overall_avg_tokens_per_trace']:.1f}")
