...
MODEL_CONFIGS = {
    # "Qwen/Qwen3-8B": {
    "/data/yichen/wyc/qwen3-1.7b-instruct": {
        "temperature": 0.6,
        "top_p": 0.95,
        "top_k": 20,
        "max_tokens": 32000,
...
MODEL_NAME = "/data/yichen/wyc/qwen3-1.7b-instruct"  # Change this to your desired model
SAMPLES_PER_QUESTION = 4  # Number of traces to generate per question
DATASET_FILE = "/home/yichen/verl/data/aime2024/test.jsonl"  # Input dataset file
...
print(f"Server URL: http://{LOCAL_HOST}:{CONFIG['server_port']}/v1")
...
client = openai.OpenAI(
    api_key="None",
    base_url=f"http://{LOCAL_HOST}:{CONFIG['server_port']}/v1",
    timeout=None
)
...
try:
    response = requests.get(
        f"http://{LOCAL_HOST}:{CONFIG['server_port']}/v1/models",
...
def generate_single_trace(question_meta, trace_idx, output_dir):
    ...
    try:
        ...
        # processed_file = '/home/yichen/verl/data/aime2024/test.jsonl'
        with file_lock:
